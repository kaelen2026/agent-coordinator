...
本地基建：`pnpm infra:up` 起 **PostgreSQL 17 + Redis 7**（compose.yaml，带健康检查，`--wait` 等就绪）；`pnpm infra:down` 停；`pnpm infra:reset` 清空数据重建。连接串复制 `.env.example` 为 `.env`（不入库）。集成测试用真库跑（testing 规则），依赖它先起。`.env` 里 `BETTER_AUTH_SECRET` 需自行生成（`openssl rand -hex 32`），留空则 api 启动即失败；`NODE_ENV=production` 时 `BETTER_AUTH_URL` / `AUTH_TRUSTED_ORIGINS` / `AUTH_TRUSTED_PROXIES` 也必须显式配置（分别决定 cookie 的 Secure 属性、CSRF 信任清单、限流分桶，兜底成开发值会静默降低安全性）。数据库 schema 由 drizzle 管理：改 `apps/api/src/modules/*/schema.ts` 后 `pnpm --filter=@agent-coordinator/api db:generate` 生成迁移、`db:migrate` 应用（迁移文件入库）。
